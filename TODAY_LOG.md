...
### 2025-12-xx 追加ログ（バッジ更新のテスト追加）
- 作業: ui-logicテストにデバイス同期バッジの表示切替（clean→dirty）を検証するケースを追加し、共通ハンドラの動作を固定化。
- テスト: `node --check js/ui/ui-controller.js`; `node js/test/ui-logic.test.js`（Pass）。
...
### 2025-12-xx 追加ログ（接続フロー統一）
- 作業: ヘッダーとレース行の接続ボタンが同じUIフロー（connectBLEUi）を通るように統一し、接続/切断/自動同期の処理を共通化。バージョンを `v2.1.0-beta.151` に更新。
- テスト: `node --check js/ui/ui-controller.js`; `node js/test/ui-logic.test.js`（Pass）。
- 感想: 行接続とヘッダー接続の差分による表示ずれリスクを減らした。接続失敗/自動同期失敗パスのスタブ化は今後の課題。
- E2E確認手順メモ（手動）: ヘッダーとレース行どちらの接続ボタンでも接続/切断後に同じ表示/バッジ更新が行われることを確認する。従来のSTART/STOP/同期確認も継続。

### 2025-12-xx 追加ログ（接続失敗パスのテスト追加）
- 作業: connectBLEUiをexportし、接続失敗時にヘッダー/接続ボタンが未接続表示のままになることをui-logicテストで検証（挙動変更なし、バージョン据え置き）。
- テスト: `node --check js/ui/ui-controller.js`; `node js/test/ui-logic.test.js`（Pass）。
- 感想: 失敗パスもテストで検出できるようになり、接続フィードバックの抜け道を減らせた。次は自動同期失敗のスタブ化が課題。
- E2E確認手順メモ（手動）: 接続失敗（タイムアウト等）時にもヘッダー/接続ボタン/設置バッジが未接続表示を維持することを画面で確認する。従来の手順も継続。

### 2025-12-xx 追加ログ（ヘッダーonclickのリスナー化）
- 作業: ヘッダー接続ボタンとモード切替ボタンのonclickを廃止し、初期化時にイベントリスナーでbindする形に移行。共通フローconnectBLEUiに統一。バージョンを `v2.1.0-beta.152` に更新。
- テスト: `node --check js/ui/ui-controller.js`; `node js/test/ui-logic.test.js`（Pass）。
- 感想: まずヘッダー部分からonclickを外せた。レース行やモーダル等の残りは段階的に進める。自動同期失敗パスのスタブ化と残onclick除去が今後の課題。
- E2E確認手順メモ（手動）: ヘッダー接続ボタンがリスナー経由でも接続/切断でき、モード切替が動作することを確認する。従来の未接続アラート→レース設定送信→START/STOPのスモークも継続。

### 2025-12-xx 追加ログ（リスナー化拡大とバージョン更新）
- 作業: ヘッダーの接続/モード切替に加え、設定/デバイス主要ボタンをイベントリスナーでbindする形に移行（onclick依存を削減）。バージョンを `v2.1.0-beta.153` に更新。
- テスト: `node --check js/ui/ui-controller.js`; `node js/test/ui-logic.test.js`（Pass）。
- 感想: onclick除去を段階的に進める土台を作れた。残りのモーダル/色選択/セグメント系onclickは次のバッチで対応予定。自動同期失敗パスのスタブ化も続き。
- E2E確認手順メモ（手動）: ヘッダー接続/モード切替と設定/デバイスボタンがリスナー経由でも動作することを確認する。従来スモークも継続。

### 2025-12-xx 追加ログ（バージョン更新 v2.1.0-beta.154）
- 作業: 確認を容易にするためバージョンを `v2.1.0-beta.154` に更新（機能変更なし）。
- テスト: 変更なし（前段の`node --check js/ui/ui-controller.js`; `node js/test/ui-logic.test.js` Passを維持）。
...
- 作業: device-managerでdummyをフィルタせず復元するようにし、再読込後もdummy配置を保持できるよう変更。バージョンを `v2.1.0-beta.149` に更新し、STATUS/NEXT/TEST_PLAN/REQUEST_ACTIONS_LOGを同期。
...
- 作業: バージョンモーダルに「接続時に設置情報を自動同期」チェックを追加し、接続/再接続後にdevice-list（dummy含む）を自動同期できるようにした。バッジ/ボタン文言を「レース設定送信」に統一。バージョンを `v2.1.0-beta.149` に更新し、STATUS/NEXT/TEST_PLAN/REQUEST_ACTIONS_LOGを同期。
