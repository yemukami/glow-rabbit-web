...
## 2025-12-xx (LLM起因のデグレ混入に対する戒め)
- 事象: renderer分離中にimport漏れ・未定義関数参照を混入させ、アプリが起動しなくなるエラーを発生させかけた（bindTargetInput未import、PaceCalculator.formatTime呼び出し）。
- 原因: モジュール分割時の依存確認不足と、構文チェックだけでブラウザ初期化パスを十分検証しなかった。LLM判断での小変更を連続させた際の確認の甘さ。
- 再発防止: 1) 分割ごとにimport/依存の棚卸しをチェックリスト化し、ブラウザ初期化パスに該当する変更は最低限の手動起動確認またはユニットテストを追加。2) 既存テストに不足するDOM/ブラウザ依存箇所はスタブテストを先に追加してから移動する。3) 連続小変更でも毎回git status確認・push前に短いスモーク（起動/主要画面遷移）を明示。
- 確認: import漏れ修正後に `node --check js/ui/ui-controller.js`、formatTime呼び出し修正後に `node --check js/ui/segment-utils.js` を再実行。ブラウザ起動エラーが解消されることをエラーログで確認済み（手動E2Eは接続環境後に実施予定）。
