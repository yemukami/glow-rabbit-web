...
- 要求: リファクタリング完了までマージせず進めつつ、挙動を維持したままリスナー化とバージョン更新を行うこと。
  対応: モーダルの色選択/タブ/セグメント追加/保存・削除ボタンとバージョンモーダルをイベントリスナー化し、同期ボタン押下時にraceIdとpacer数をログ出力するよう追加（SYNC_START_SPEC順守・挙動変更なし）。バージョンをv2.1.0-beta.154に更新し、STATUS/NEXT/TEST_PLAN/TODAY/REQUEST_ACTIONS_LOGを同期。`node --check js/ui/ui-controller.js`, `node js/test/ui-logic.test.js` を実行（Pass）。コミット/プッシュ済み（branch: refactor/event-listener-migration）。

- 要求: バージョンを上げて確認しやすくすること。
  対応: v2.1.0-beta.154に更新し、リスナー化した設定/デバイス/ヘッダー操作を維持したままログ/ステータスを同期（SYNC_START_SPEC順守・挙動変更なし）。
...
- 要求: リスナー化を進めつつ同期/接続フローの安全性を保ち、ログ/テスト/プッシュまで行うこと。
  ...
  追記: 確認しやすいようバージョンをv2.1.0-beta.154に更新（挙動変更なし）。
...
- 要求: リファクタリングの優先度を考えつつ、接続/同期ステータスの共通化を進め、テスト/ログ/プッシュまで行うこと。
  対応: updateConnectionStatusをエクスポートし、ui-logicテストに接続成功/切断時のヘッダー＆バッジ更新を検証するケースを追加（SYNC_START_SPEC順守・挙動変更なし、バージョン据え置き）。`node --check js/ui/ui-controller.js`, `node js/test/ui-logic.test.js` を実行（Pass）。コミット/プッシュ済み。
