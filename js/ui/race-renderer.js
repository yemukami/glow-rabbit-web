...
function buildInfoHeader(race, maxDist, safeStartPos, elapsedTime) {
    if (race.status === 'ready') {
        ...
        return `<div style="display:flex; gap:10px; align-items:center;"><div class="timer-big" style="color:#DDD;">00:00.0</div><input type="number" value="${safeStartPos}" class="input-start inline" min="0" data-action="startPos" data-race-id="${race.id}">${syncTag}</div>`;
    }
