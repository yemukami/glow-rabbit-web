# Glow-Rabbit Web App - Status & Remaining Tasks (v2.1.0-beta.155)
...
- 接続成功/失敗/切断のいずれでも共通ハンドラでヘッダー表示と設置同期バッジを更新し、初期ロード/同期完了後もdirty状態がUIに反映されるように整理。ヘッダーの接続/モード切替ボタンをリスナー化し、onclick依存を削減。
- ヘッダーとレース行の接続ボタンを同一フローに統一し、接続/切断/自動同期が共通経路を通るようにした。
...
- 入力ガード（非負/正整数）のユニットテストを追加し、サニタイズ挙動を固定化。
- startPosサニタイズ時の警告ログをテストでカバーし、raw/sanitizedが記録されることを確認。
- モード切替のDOM更新をscreen-rendererに分離し、ui-controllerのDOM責務を削減。
- レースタイトル同期もscreen-renderer経由に寄せ、ui-controllerのDOM操作をさらに削減。
- 競技タイトル→レース画面タイトル同期もrendererヘルパーに寄せてDOM操作を整理。
- モード切替ガードの順序を修正（ガード判定後にレンダリング切替）し、挙動を安定化。
- バージョン表記同期をrendererヘルパーで担保し、UI/モーダルの表記ズレを防止。
- ロード時に競技タイトルを入力とレース画面タイトルへ同期する処理をrendererヘルパー経由に移行。
- 競技タイトル反映とセグメント合計表示の更新をrenderer/viewヘルパー経由に寄せ、ui-controllerのDOM依存をさらに削減。
- デバイスグリッドの描画/カウント更新/ハンドラ接続をdevice-renderer経由に集約し、ui-controllerのDOM操作を削減。
- デバイスアクションモーダルの生成とイベント付与をdevice-rendererに寄せ、ui-controllerのDOM依存をさらに削減。
- 置換モーダルの表示/イベント付与をrenderer側に寄せ、ui-controllerのDOM操作を削減。
...
- 未接続でも画面遷移は許可し、dirty時は警告だけ出して後続で接続後に同期できるように整理。
- レース進行更新でelapsedTimeを一度計算してrendererへ渡し、二度読みを避ける軽微なリファクタ。
- checkDirtyAndSyncの文言をユーザー向けに簡潔化しつつ「続行」を明示。
...
- 進行表示更新をrendererのupdateRunningDisplaysForRaceに寄せ、ui-controllerのrenderer依存を明示。
