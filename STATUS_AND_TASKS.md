# Glow-Rabbit Web App - Status & Remaining Tasks (v2.1.0-beta.74)
...
- レースUI状態（展開行/経過時間/編集ペース）を `race-ui-state` に集約し、renderer移行の足場を整備。テーブル操作をデータ属性＋デリゲーションに置換し、行展開/アクションをUI層で一元処理。
